...
@pytest.mark.asyncio
async def test_compose_trim_numeric_value_becomes_str():
    # value may come back as a JSON number; _run_verb/parse_value want a string
    router = _FakeRouter(
        '{"verb": "trim", "sequence": "CUT", "segment_index": 5, "value": 1015}'
    )
    verb, seq, idx, val, err = await interactive._nl_compose("…", router=router)
    assert err is None and verb is verbs.REGISTRY["trim"]
    assert (seq, idx, val) == ("CUT", 5, "1015")
    # the str feeds the SAME trust-boundary parse the prompts use
    typed, perr = verbs.parse_value(verb, val)
    assert typed == 1015 and perr is None
