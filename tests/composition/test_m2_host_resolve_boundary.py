...
@pytest.mark.asyncio
async def test_host_resolve_routes_live_proven_sequence_marker_executor():
...
@pytest.mark.asyncio
async def test_host_resolve_routes_live_proven_segment_marker_executor():
...
@pytest.mark.asyncio
async def test_host_resolve_holds_transition_executor_before_live_proof():
...
async def test_host_resolve_routes_live_proven_split_executor():
    ...
    result = await HostResolveBoundary(run_discover=run_discover).dispatch(
        ...
        {
            "deltas": _upstream_result(
                executor="forge_apply_segment_split_delta"
            )
        },
    )
