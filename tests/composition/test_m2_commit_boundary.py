...
class _AdvertisedPositionMCP(_RenameMCP):
    ...
    async def call_tool(self, name: str, arguments: dict):
        self.calls.append((name, copy.deepcopy(arguments)))
        if name != "forge_apply_segment_position_delta":
            raise AssertionError(name)
        mode = arguments["mode"]
        if mode == "verify":
            return copy.deepcopy(self._fresh_manifest)
...
class _AdvertisedSplitMCP(_RenameMCP):
    ...
    async def call_tool(self, name: str, arguments: dict):
        self.calls.append((name, copy.deepcopy(arguments)))
        if name != "forge_apply_segment_split_delta":
            raise AssertionError(name)
        mode = arguments["mode"]
        if mode == "verify":
            return copy.deepcopy(self._fresh_manifest)
...
class _AdvertisedSequenceMarkerMCP(_RenameMCP):
    def __init__(self, *, fresh_manifest: dict):
        super().__init__(fresh_manifest=fresh_manifest)
        self.list_count = 0
    ...
    async def list_tools(self):
        ...
        return [
            SimpleNamespace(
                name="forge_apply_sequence_marker_delta",
                inputSchema={"type": "object", "properties": {}, "required": []},
            )
        ]
    ...
    async def call_tool(self, name: str, arguments: dict):
        self.calls.append((name, copy.deepcopy(arguments)))
        if name != "forge_apply_sequence_marker_delta":
            raise AssertionError(name)
        mode = arguments["mode"]
        if mode == "verify":
            return copy.deepcopy(self._fresh_manifest)
...
class _AdvertisedTransitionMCP(_RenameMCP):
    ...
    async def call_tool(self, name: str, arguments: dict):
        self.calls.append((name, copy.deepcopy(arguments)))
        if name != "forge_apply_transition_delta":
            raise AssertionError(name)
        mode = arguments["mode"]
        if mode == "verify":
            return copy.deepcopy(self._fresh_manifest)
...
@pytest.mark.asyncio
async def test_commit_boundary_runs_live_proven_sequence_marker_counterpart():
    tool_name = "forge_apply_sequence_marker_delta"
    held = _held_manifest_dict()
...
@pytest.mark.asyncio
async def test_commit_boundary_runs_live_proven_transition_counterpart():
...
@pytest.mark.asyncio
async def test_commit_boundary_runs_live_proven_position_counterpart():
    held = _held_manifest_dict()
...
@pytest.mark.asyncio
async def test_commit_boundary_runs_live_proven_split_counterpart():
    held = _held_manifest_dict()
    ...
    assert mcp.list_count == 1
    assert [(name, args["mode"]) for name, args in mcp.calls] == [
        ("forge_apply_segment_split_delta", "verify"),
        ("forge_apply_segment_split_delta", "apply"),
    ]
