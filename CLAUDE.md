...
- **v1.9 Conversational Reads** — shipped capability-gap declines, deterministic complete shot-name lists, and exact project-referent grounding alongside the `(b)` terminal-synthesis answer-pass at the `chain_complete` read seam (`acomplete`, local Ollama). Reads-only by structure; mutations stay deterministic preview+ratify. The tagged runtime passed 3/3 live planner-front UAT cases. Organic chain-corpus capture is active; issue #102 stays open until the reader certifies real-distribution coverage.
...
│   ├── composition/            ← Graph-native execution runtime (GraphSpec IR, pure GraphExecutor, UnifiedDispatch→boundaries, admission table, legacy-vs-graph compare harness) — M1/M2; own __all__; Pipeline v3 production semantic callers are live
...
Milestone: **v1.9 Conversational Reads** is shipped; no new implementation milestone is active. The current mode is operator/user dogfood. There is no single live state cursor — continuity is distributed across milestone/phase docs under `.planning/`, `.planning/JOURNAL.md`, and git history; `.planning/CONTINUITY-MAP.md` maps which artifact carries what authority. The most recent milestone-close doc plus that map is the cold-resume surface.

**Current graph frontier — dogfood and evidence, not another preselected build slice.** Pipeline v3 supplies real production semantic callers for Bridge `GraphSpec`; its live orchestration matrix passed trusted across all six supported hosts. The older chat/NL path is a distinct migration and still uses `run_chain_steps`. Its capture source is implemented and persistently enabled with `FORGE_BRIDGE_CHAIN_CORPUS_CAPTURE=1`; #102 now waits for organic distribution coverage from `chain_corpus.reader.coverage_report`, not a hand-authored seed. Once that gate is satisfied, #93 owns parity-reader cutover and legacy-path retirement. Keep #88 deferred until independent expensive fan-in is real, #142 optional until budget/count assent is demonstrated pain, and #188 deferred until the full v1.9 compatibility cycle has elapsed. NB the planning archaeology carries several overlapping naming schemes (v1.x milestones · M1/M2 composition · JOURNAL "Phase N/25.x" / "v1.12") — the per-milestone close docs + `CONTINUITY-MAP.md` reconcile authority; do not assume one scheme subsumes the others.

**v1.9 result:** the read path now answers humans while keeping facts substrate-grounded. The release includes the `(b)` terminal-synthesis pass, deterministic complete shot-name rendering, exact project-referent grounding, and honest capability-gap declines; the tagged runtime passed 3/3 live planner-front UAT cases. Organic dogfood now feeds the active chain-corpus capture for #102. Mutations remain deterministic preview → ratify → apply.
