...
- **Conda env vs. daemon env confusion.** The daemons run as system services with absolute python paths from the `forge` conda env. `conda activate forge` is for *your shell* (so `fbridge` resolves) — it doesn't affect the running daemons. After editing `/etc/forge-bridge/forge-bridge.env`, restart the daemon: `sudo systemctl restart forge-bridge` (Linux) or `fbridge restart console` (macOS).
