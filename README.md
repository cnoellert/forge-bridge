...
curl -fsSL https://raw.githubusercontent.com/cnoellert/forge-bridge/v1.9.15/scripts/install-flame-hook.sh | bash
```

The installer copies `forge_bridge.py` into `/opt/Autodesk/shared/python/forge_bridge/scripts/` and sanity-checks the result. Override `FORGE_BRIDGE_VERSION` (default `v1.9.15`) or `FORGE_BRIDGE_HOOK_DIR` (default the Flame shared-python path) as needed.
