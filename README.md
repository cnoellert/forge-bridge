...
Shipped at **v1.9.18** (2026-07-31). v1.9.18 governs paired render and workfile promotion (Phase 160): one ratification drives staged commits across resource copy, catalog registration, main workfile promotion, and lineage, with the lineage plan discovered only after its Version authorities exist and verified to name the exact committed IDs. v1.9.17 verifies and forwards Pipeline's plugin-owned editorial transaction host entries without deriving host mutations from complete semantic deltas; per-entry, ordered-list, and outer realization fingerprints all fail closed before host contact. v1.9.16 admits Pipeline's read-only `shot_resource_publish_plan` operation through the released Flame hook, allowing selected-role publish previews to compile against the live DCC before any mutation. v1.9.15 exposes the typed Flame workfile lifecycle route through the released hook, avoids nested Flame main-thread scheduling, and preserves immutable Version names on fresh catalog reads so Pipeline Phase 156 can validate a saved stream before publish. v1.9.9 added the closed editorial-edit workflow API (Phase 149): one durable propose → ratify_apply → status → replay → restore product surface over the exact-realization + AssentRecord + commit machinery, returning path-free, canonically-fingerprinted receipts. v1.9.10 admits the governed split restore: it captures the split apply's closed recovery token, discovers a fresh `forge_apply_segment_split_restore` manifest, and commits a separately ratified inverse through the same verify-before-apply boundary (drift, missing token, and apply failure all fail closed). v1.9.11 preserves the immutable original receipt for an exact duplicate `propose` — even after the durable workflow has advanced past the propose state, the returned receipt stays `proposed` with no commit fingerprint. v1.9.12 adds the durable workspace publish workflow API (Phase 156): a closed propose → ratify_apply → status → replay → propose_recovery → ratify_recovery surface that composes Pipeline's publish transaction into `commit`, holds the discovered manifest behind a proposed `AssentRecord`, and gates recovery on a read-only inspect before a separately ratified abort. v1.9.13 adds the governed editorial transaction lifecycle (Phase 153): two-to-eight ordered commands admitted as one aggregate host mutation under a single assent, with the host-generated recovery token captured verbatim and a separately ratified restore through the same verify-before-apply boundary. v1.9.14 adds the durable workspace main-promotion workflow API (Phase 156): one `AssentRecord` governs a three-commit stream-to-main chain — physical resource copy, promoted-resource catalog registration, and immutable main Version/Media registration — each proved separately in a closed receipt carrying three stage statuses and six manifest/commit fingerprints. `partial_failed` is a real terminal disposition, and replay is resumable rather than observational: per-stage durable status lets it forward-complete over the original proposal, graph, assent, and held manifests without re-copying bytes or minting a second main Version. Conversational Reads distinguishes missing information from missing capability, renders complete pure shot-name lists without a second model pass, and grounds directly named projects to exact registry identities. The v1.9.1 patch normalizes quoted prefixes and Flame reel names; v1.9.2 keeps current `websockets` compatibility; v1.9.3-v1.9.4 admit Pipeline load, refresh, and version-switch commit counterparts; v1.9.5 adds reviewed peer-owned stream-promotion copies; v1.9.6 composes the post-copy validation and registration plan into a separate verify-before-apply, assent-required Bridge-owned catalog commit; v1.9.7 admits Pipeline's atomic publish transaction under explicit federated transaction authority; and v1.9.8 adds read-only transaction status plus held, peer-owned safe abort. Milestones v1.5 Legibility (install + concept docs + recipes + diagnostics), v1.6 Operability (graph-native operational runtime, doctor observability, chat-layer convergence), v1.7 Artist Readiness (NL → compile → preview → ratify → apply authority chain), and v1.8 Console Authority (ratification projection onto the Console) have all shipped since the v1.4.1 baseline. Public `forge_bridge.__all__` remains **19 symbols** — the surface has stayed stable while behavior deepened.
...
curl -fsSL https://raw.githubusercontent.com/cnoellert/forge-bridge/v1.9.18/scripts/install-flame-hook.sh | bash
```

The installer copies `forge_bridge.py` into `/opt/Autodesk/shared/python/forge_bridge/scripts/` and sanity-checks the result. Override `FORGE_BRIDGE_VERSION` (default `v1.9.18`) or `FORGE_BRIDGE_HOOK_DIR` (default the Flame shared-python path) as needed.
