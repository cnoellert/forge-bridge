...
- [x] **Phase 16.2 (FB-D Bug D closure): Chat Tool-Call Loop + Fresh-Operator UAT** (INSERTED) — Close the Bug D gap surfaced in Phase 16.1's UAT on assist-01 (2026-04-27): chat surface returns raw tool-call JSON `{"name": "forge_tools_read", ...}` as assistant text instead of executing the chosen tool and synthesizing a useful answer. Investigate three plausible loci (`LLMRouter.complete_with_tools()` agentic loop, chat handler tool dispatch via `mcp.call_tool()`, UI rendering of structured tool_calls), strengthen `tests/integration/test_chat_endpoint_live.py` so it cannot pass on a tool-call-only response, and re-run a fresh-operator UAT to satisfy CHAT-04 and unblock v1.4 milestone close (inserted 2026-04-28 per Phase 10/10.1/16.1 precedent) — **Plans: 4 plans across 3 waves** (completed 2026-04-28)
