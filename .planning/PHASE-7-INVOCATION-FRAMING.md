...
**V1 RESOLVED — DT-verified + pushed.** Creative's architectural read confirms the seam was crossed *for real*, not tunnelled around: B→A→C identity chain real, backend resolution real, refusal path real, provenance stamp real, reverse-resolution real — only the backend itself is stubbed (exactly where the fake belongs). The previously-theoretical invocation spine now has concrete shape (discover→select→dispatch→artifact→poll→terminal). The question shifts from *"how should invocation work?"* to *"where should it attach?"* → Vertical 2.
