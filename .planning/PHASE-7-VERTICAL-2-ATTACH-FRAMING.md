...
**Status:** **V2 LANDED `447fc8d`, pending DT verify.** B ratified (event-driven, execution-stage event → dispatch consumer) with Creative's two-seam split. Brief: `PHASE-7-VERTICAL-2-ATTACH-BRIEF.md`. Grounded against `main @ ffb7bc9`. Follows V1 (`6d22d14`, RESOLVED). Parent: `PHASE-7-INVOCATION-FRAMING.md`.

> **V2 LANDED `447fc8d` (code self-report):** `DispatchOnExecutionEntryConsumer` consumes `stage_advanced` events with `to_stage="execution"` → `dispatch_plan` (trigger event_type resolved = `stage_advanced`). Shared `GenerationDriverRegistry` across dispatch + poll. Replay-driven full round-trip test (replay→execution event→consumer dispatch→submitted artifact→poll terminal) + negative (`dispatch_no_driver`, no artifact). **Grep proof held — `dispatch_plan(` only in def + consumer + tests** (the success criterion: no callsite dispatch logic). 45 focused passed; full `2857 passed/42 skipped` (importlib); ruff clean; `__all__`=19.
>
> **For DT verify (3):** (1) **the property, not just the round-trip** — confirm the grep proof genuinely holds and dispatch fires *only* via the consumer (automatic consumption on state entry). (2) **The clean attach is real, not under-stressed** — code reports NO hidden assumption surfaced; Creative expected the integration to surface what breaks, so confirm the round-trip runs through replay's *real* execution transition (not a shortcut), i.e. "nothing broke" = the event-consumer symmetry paid off, not a test that avoided the integration ([[feedback_fixture_shape_mirrors_production]]). (3) **Exactly-once idempotence** on execution entry (the framing's open item) — dispatch fires once per entry, survives re-poll/replay-of-replay.
