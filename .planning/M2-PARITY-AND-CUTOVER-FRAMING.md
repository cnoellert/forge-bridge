...
> **RATIFIED-STRICTER (slice-1 verify, 2026-06-18):** the original wording was *"the executor carries `assent_record` as opaque payload — it never inspects, decides, or mints it."* The slice-1 implementation chose a **cleaner design and the room ratified it**: **assent flows through the dispatch closure; the executor never references assent at all.** The graph executor has *no* assent surface — assent lives in the orchestration-built dispatch closure (the same place the `AbortOnFirstError` wrapper lives), never as an executor param. This is strictly better than "carries opaque payload" (the executor can't mis-handle what it can't name) and it supersedes the original wording. **Slice-3's author must NOT add an assent param to the executor** — thread assent through the dispatch closure.
>
> preview→ratify→apply orchestration stays in `_operation_front.py` / `_chat_compile.py` / `handlers.py` / `core/assent.py` and hands assent to the graph path (via the closure) instead of to `run_chain_steps`. The "cannot regress ratify byte-for-byte" guarantee is then auditable as *the same orchestration code now driving `GraphExecutor`* — not as authority logic absorbed into a node.
...
**Enforcement — grep-auditable CI invariant (DT; ratified-stricter at slice-1 verify):** the slice-1 guard is a **total token ban** in `executor.py` — `AssentRecord` / `assent_record` / `ratified` / `ratification` must not appear *at all* (`tests/composition/test_m2_executor_invariants.py`). This is stricter than the original "zero attribute-access/conditional, may forward" wording and encodes the ratified design: the executor has no assent surface whatsoever. Trivially green today; it bites the instant slice 3 tries to give the executor an assent param.

**Scope (corrected in slice-1 seam design):** the CI grep is scoped to **`forge_bridge/composition/executor.py` specifically — NOT all of `forge_bridge/composition/`**. The boundary (`boundary.py`) legitimately interprets tool results, so a blanket `composition/` ban would false-positive on it. Plant the guard in slice 1 (trivially green — the executor has no `assent_record` today) so it bites the instant slice 3 introduces an inspection. See [[M2-SLICE-1-SEAM-DESIGN]] Seam S1-D.
...
- **Seam B** — RATIFIED-STRICTER at slice-1 verify: assent flows through the dispatch closure; the executor **never references assent at all** (better than "opaque payload" — no assent surface to mis-handle). CI invariant = total assent-token ban scoped to **`executor.py`** (`test_m2_executor_invariants.py`). Slice-3 must thread assent via the closure, not an executor param.
