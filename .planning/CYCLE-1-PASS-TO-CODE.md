...
3. **`workfile` — eventual home intentionally UNBOUND; do NOT pre-route to the role axis** (Creative redline 2026-06-07). The room has not determined whether "editable-source vs produced-output" is a role distinction at all. Four open candidates: media-role member / third `role_class` / a distinct **lifecycle** dimension / an **edge property** (produced-output = target of `produces`; source = a different edge — live given edge-over-attribute + workfile already on a `version_of` edge). The axis is **already half-collapsed today**: media `role_class` carries `generation_floor` (`vocabulary.py:167-181`, raw=0 source / rest=1+ produced). 2nd source artifact = evidence, NOT proof. Do NOT add a `source`/`workfile` `role_class` when the next source artifact appears — only once confident editable-vs-produced belongs on the role axis at all. Full framing in `HANDOFF-2026-06-07-CYCLE-1-AND-PIPELINE.md`.
