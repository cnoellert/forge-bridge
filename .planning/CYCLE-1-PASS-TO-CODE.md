...
3. **`workfile` classification is an OPEN deliberate decision** (known media member vs a third `role_class`). It's a DCC scene file, not rendered output — semantically distinct. Open-membership today (not rejected), so no rush. Decide with the room in Part C/Phase 40; bump the contract then. Do NOT slip it into v0.2 reactively (`role_class` is the closed axis — a third class is the highest-stakes vocab change).
