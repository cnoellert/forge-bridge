...
- **D-17:** ~~Tool registration site is `forge_bridge/mcp/registry.py::register_builtins()` for the four `forge_*_staged` tools (alongside the existing `forge_list_*` / `forge_get_*` family).~~ **Revised 2026-04-26 after research (Open Question #2 — Solution C):** all four `forge_*_staged` tools register from `forge_bridge/console/resources.py::register_console_resources()` alongside the resource and shim, NOT from `register_builtins()`. Reason: `register_builtins(mcp)` runs at import time without `console_read_api` or `session_factory` in scope, but the four staged tool bodies need both. Solution C uses the same closure-capture site that already wires `forge_manifest_read` — no new state-management pattern, no module globals. The `forge_staged_pending_read` shim and `forge://staged/pending` resource also register from `register_console_resources()` (their original site per CONTEXT). Net effect: ALL FB-B MCP surface (4 tools + 1 resource + 1 shim) registers from one site.

- **D-17a (NEW):** Plan 14-01 includes a one-line fix to `forge_bridge/store/staged_operations.py:289` to address WR-01 (`from_status="(missing)"` sentinel that prevents FB-B handlers from cleanly distinguishing 404 from 409). The fix replaces the sentinel with a proper not-found-typed exception (or adds a `not_found: bool` discriminator field on `StagedOpLifecycleError`). Reason: alternative was coupling FB-B handlers to a private FB-A sentinel string — worse contract.
