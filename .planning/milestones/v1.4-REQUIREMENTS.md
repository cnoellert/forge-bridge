...
| LLMTOOL-01 | FB-C | Closed | Direct sentinel UAT PASSED live 2026-04-28 in 6.39s on assist-01 against qwen2.5-coder:32b (`test_ollama_tool_call_loop_live`). Required closing Phase 16.2 REVIEW.md WR-01 inside the UAT loop — original salvage helper required content to parse as pure JSON, but qwen2.5-coder:32b emitted `{"name": ..., "arguments": ...}\n<fabricated answer>` (trailing-prose Bug D variant). Widened `_try_parse_text_tool_call` to handle markdown-fenced + leading-prose + trailing-prose variants via `json.JSONDecoder().raw_decode()`. Local LLM tool-call parity with Claude path is now real, not aspirational. |
