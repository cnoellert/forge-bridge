...
### D2 — Step-2.1 adoption: clean swap to published types? **[Orch lean: YES, low-risk] — RESOLVED `02293f6`, DT verify PASSED**
...
**RESOLVED `85ce467` (brief: `PHASE-6A-RUNG-2B-D4-BRIEF.md`).** pass_3 routes through `by_family("perception")` (matte still fallback); test fixture un-masked to `family="perception"`; discriminating regression added (perception wins over matte when both registered) + **verified to fail under the old source by selecting matte** — the unmask proof. **Functional** residual `perceptual` (live code paths + active fixtures) → **zero**. `pytest test_planner + test_sibling_registration + test_sibling_discovery_live` → 56 passed / 1 skipped; ruff clean; `forge_bridge.__all__` → 19. Equivalence class closed (1 divergent literal + 1 mask → 0 functional residual).

**DT verify — PASSED, with one doc-provenance correction folded in.** DT confirmed every behavioral claim (fix site, discriminating regression fails-on-old-source, fixture unmasked, suite 56/1, ruff, `__all__` 19). The one finding: `85ce467` over-scrubbed an *archaeology* comment in `discovery.py:29` — a line whose job is to quote the original `perceptual` **typo** that doomed the retired allowlist; "correcting" it to `perception` made the comment self-refuting. Restored (`(sic)`-marked) so the typo-evidence lands intact. The grep-zero invariant is for *functional* residuals only ([[feedback_doc_provenance_discipline]] · [[feedback_grep_c_completion_invariant]]). **D4 RESOLVED.**

> **Superseded count (D2, `02293f6`):** the inline `(sic)` archaeology comment was removed when D2 relocated the request-all guidance to the construction site — a legitimate refactor, not a scrub. `perceptual` now reads **0** across `forge_bridge/` + `tests/`. The typo provenance lives durably in `PHASE-6A-DISCOVERY-ALIGNMENT.md` § "Family vocabulary divergence" (the archaeology layer, immune to code-refactor churn) — so no evidence was lost. Inline `(sic)` deliberately **not** re-added: it would only re-arm the next grep-zero pass.
...
- **D2 RESOLVED `02293f6`** — Step-2.1 de-shadow; DT verify PASSED (live integration green). Bridge + siblings now share one registration context type.
- **D3 RESOLVED = C (close motion 2)** — convergence ran (`PHASE-6A-RUNG-B-CONVERGENCE.md`): A rejected unanimously on burden-of-proof; key sharpening = B′ pays the same planner ripple as A, so the choice was pay-rename-now vs defer; operator ruled C (*"capability is implied"* — no felt friction). **B′ deferred with a live re-entry trigger** (shovel-ready rename when the `tool_id`/`capability_id` divergence bites in ordinary workflow). **Motion 2 (registry-type separation) is closed.**
