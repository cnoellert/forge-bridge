...
**Status:** **V3 RESOLVED `b68ef67` — DT-verified (test/autonomy level) + pushed.** ⚠ **One operational follow-up outstanding:** the live `fbridge up` daemon smoke (real process comes up clean with the 3 always-on tasks, `doctor` healthy, no log spew, benign idle) was NOT in DT's report — DT covered the bootstrap path via the autonomous in-process round-trip. Pushing main is safe (daemons hold open handles until restarted); **run the live smoke as a pre-restart gate before portofino/flame-01/assist-01 pull onto this commit** ([[feedback_caplog_limitation]]). DT autonomy proof: the round-trip test injects the stub then `_wait_until(stage=="audit")` — daemon-started workers carry dispatch→poll→terminal→audit unaided (genuinely autonomous, not hand-driven). D1 = full execution-runtime set (Creative ruling; DT direction; Orch concur). Started in `:9996` lifespan; one shared registry (empty in prod); stub injected only for the proof. Boundary: *"V3 establishes the runtime; it does not expand the federation."* Brief: `PHASE-7-VERTICAL-3-OPERATION-BRIEF.md`. Grounded against `main @ 1842996`. Follows V2 (`447fc8d`, DT-verified). Parent: `PHASE-7-INVOCATION-FRAMING.md`.

> **V3 LANDED `b68ef67` (code self-report):** `bootstrap_daemon` starts the runtime set (dispatch consumer + poller + terminal consumer) as managed tasks; one shared `GenerationDriverRegistry` empty in production; teardown cancels in reverse. Dispatcher now carries `run_id` (when the execution-entry consumer has it) so terminal artifacts advance the lifecycle to audit. Daemon proof test (bootstrap starts tasks → proof injects stub → replay → autonomous dispatch→poll→terminal→audit) + empty-registry graceful-degradation test. 31 focused passed; full `2859 passed/42 skipped`; ruff clean; `__all__`=19; dispatch + execution-transition grep guardrails held.
>
> **For DT verify — V3 changes PRODUCTION startup, so the bar is higher than a test pass:**
> 1. **Autonomy, not harness** — the round-trip is carried by the *bootstrap-started* tasks; the harness only injects the stub + triggers replay + observes (same "not bypassed" check as V1/V2). Confirm.
> 2. **Live daemon smoke (highest value for an operational rung)** — a green suite ≠ a healthy daemon ([[feedback_caplog_limitation]]). Start a real daemon (`fbridge up` / launchd restart), confirm it comes up clean with the 3 new tasks, `fbridge doctor` healthy, no error spew in the real log, and the empty-registry tasks idle benignly (sane poll intervals, not hammering the DB/event store). This is the "survives contact with the actual daemon lifecycle" proof V3 is *for*.
> 3. **Graceful-empty in prod** — stock daemon (no drivers) → consumer runs, dispatch emits `dispatch_no_driver`, loop survives; nothing crashes startup.
> 4. **Clean teardown** — shutdown cancels the 3 tasks, no orphans/errors; `tests/console/test_lifespan_wiring.py` still green (new tasks don't regress the documented startup sequence).
> 5. **The `run_id` threading** code added — verify it's sound and doesn't regress V1/V2 (the run_id flows so the terminal consumer advances to audit).
...
### D3 — Registry ownership + the empty-registry wrinkle **[Ruled: lifespan owns one shared empty registry; proof injects the faithful stub]**
One `GenerationDriverRegistry` created in the lifespan, injected into dispatch consumer + poller (V2's shared-registry, daemon-scoped). **Wrinkle (grounded):** a stock daemon has **no drivers registered** (real generation drivers come via the deferred Generators-adapter rung, V1 Q1) — so dispatch emits `dispatch_no_driver`. For V3's alive-proof, the test injects the V1 faithful-lifecycle stub into the bootstrap-created registry so the round-trip completes through the real daemon tasks. Wiring the *real* Generators driver stays the deferred rung. (This keeps V3 about *aliveness*, not about real backends.)
...
## Scope (CANDIDATE)
- **IS:** start the execution-runtime worker set in the `_lifespan`; one daemon-scoped shared registry, empty in production; inject the stub driver only in the proof; prove the full generation round-trip through the actual daemon (triggered via replay — the path that reaches execution); graceful shutdown.
...
Start the **full execution-runtime set** in the lifespan with a daemon-scoped shared registry, keep production empty, inject the stub only in the proof, and prove the replay-triggered round-trip end-to-end **in the running daemon, no harness**. That's the truest form of "is it alive," and the grounding correction (nothing runs today) means we're establishing the runtime regardless — better to establish the coherent set than a lone consumer that can't round-trip.
