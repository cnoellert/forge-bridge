...
**v1.5 ship blocker — CLOSED (2026-05-01):** Phase 20.1 shipped the install bootstrap script + systemd units + launchd plists + env-file template + INSTALL.md reshape across 8 plans (20.1-01 through 20.1-08, the latter being a gap-closure plan that added MCP HTTP transport mode). Author-walked Linux gate (flame-01) and macOS gate (portofino) both PASS-with-deviations per D-19. All 12 Phase 20 functional gaps closed; the daemon model holds (`:9996` Console + chat + Read API, `:9997` MCP HTTP, `:9998` WS bus all persistently bound; FastMCP user lifespan composed into the Starlette ASGI app for non-stdio transports — commit `7fe9779`). 6 in-flight gaps surfaced during the walks; 5 patched in-flight per D-20 (commits `aaf6965`, `7fe9779`, `c73227f`, `c9aac41`, `cb687f8`), 1 deferred. Two SEEDs planted at close: `SEED-PHASE-20.1-ARTIST-UAT-V1.6+` (real-artist fresh-machine walk to earn clean PASS at v1.6+) and `SEED-PHASE-20.1-POSTGRES-OWNERSHIP-V1.6+` (decouple cluster ownership from schema ownership; the Autodesk-bundled Postgres on `:7533` reality drove this). Verification: `.planning/phases/20.1-install-bootstrap-script-systemd-daemon-v1-5-ship-blocker/20.1-VERIFICATION.md`.
...
---
*Last updated: 2026-05-01 — Phase 20.1 (Install Bootstrap Script + Systemd Daemon, v1.5 ship blocker) CLOSED. Both gates PASS-with-deviations (Linux flame-01 + macOS portofino) after 5 in-flight bootstrap fixes per D-20 + Plan 20.1-08 lifespan composition. v1.5 ship blocker resolved. Two close-time SEEDs (`SEED-PHASE-20.1-ARTIST-UAT-V1.6+`, `SEED-PHASE-20.1-POSTGRES-OWNERSHIP-V1.6+`) capture the v1.6+ structural follow-ups (real-artist UAT; decouple cluster ownership). Verification: `.planning/phases/20.1-install-bootstrap-script-systemd-daemon-v1-5-ship-blocker/20.1-VERIFICATION.md`.*
