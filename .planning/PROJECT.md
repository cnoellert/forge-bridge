...
**Shipped:** v1.4.0 (2026-04-28) — `v1.4 Staged Ops Platform` milestone complete. forge-bridge now ships the human-in-the-loop primitives projekt-forge v1.5 needs to become a thin Flame-side consumer: `staged_operation` entity with proposed→approved→executed/rejected/failed lifecycle + `DBEvent` audit (FB-A); MCP tools + HTTP routes + `forge://staged/pending` resource served from a single `ConsoleReadAPI` facade with byte-identical zero-divergence (FB-B); `LLMRouter.complete_with_tools()` agentic loop with provider-neutral coordinator + thin Anthropic + Ollama adapters, hard iter/wall-clock caps, repeat-call detection, 8 KB result-size cap, sanitization boundary, recursive-synthesis guard (FB-C); and a `/api/v1/chat` endpoint with rate limiting, sanitization end-to-end, and external-consumer parity that serves Web UI + projekt-forge Flame hooks identically (FB-D). Two decimal gap-closure phases were inserted mid-milestone per the Phase 10/10.1 precedent: 16.1 closed three Phase 16 deploy bugs (Starlette `TemplateResponse` signature drift, lifespan wiring guard, chat tool-list hang on full MCP registry); 16.2 closed Bug D — chat surface returning raw tool-call JSON as the assistant's terminal text instead of executing the tool. Bug D was diagnosed via captured-fixture RED test against real qwen2.5-coder traffic from assist-01, then closed at the lowest-cost layer (`OllamaToolAdapter._try_parse_text_tool_call` salvage). CHAT-04 fresh-operator UAT recorded `Outcome: PASS with deviations` per the Phase 10.1 / 16.1 D-14 precedent — three D-08 floor conditions all PASS on assist-01. See `.planning/milestones/v1.4-ROADMAP.md` for the full archive and `.planning/milestones/v1.4-MILESTONE-AUDIT.md` for the close-out audit.
