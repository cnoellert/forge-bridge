...
- **Boundary:** the *selection/dispatch route* is wrong; a correctly-extracted param sent to the wrong tool.
...
- **Ranking unit = the defect (code site), NOT the class.** Class-frequency (how often each mechanism appears)
  is a **diagnostic lens** for Phase 4; it is **not** the slice-selector, because a multi-class defect would
  double-count across its classes. Phase 3b ranks **deduped defects**; the class-frequency table is reported
...
|---|---|---|---|
| **routing** (§3.2) | `tools_filtered=1` + no compile line (D1 signal) | **1 — substrate-observable** | a legitimately single-tool message also filters to 1 (low risk; corroborate with no-compile-line) |
