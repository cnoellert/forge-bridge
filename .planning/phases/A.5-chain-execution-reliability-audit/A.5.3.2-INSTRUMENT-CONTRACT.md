...
These are the points where the contract is intentionally underspecified
because the answer should come from review:

1. **Initial seed corpus.** Should the test fixture's seed prompt list
   be checked into the repo as YAML/JSON, or generated by a fixture
   factory? My lean: checked-in, under
   `tests/corpus_seed_prompts.yaml`. Confirm or override.

2. **Runtime probe deployment surface.** Should the env var be
   `FORGE_BRIDGE_DIVERGENCE_CAPTURE=1` (binary on/off) or
   `FORGE_BRIDGE_DIVERGENCE_CAPTURE=fraction:0.1` (sample 10% of
   arbitrations)? My lean: binary for v0; revisit if corpus growth
   becomes unwieldy.

3. **Comparator's interaction with `sensitive=True` chat sessions.**
   Sensitive sessions route through local Ollama. Should the
   comparator honor that routing flag when calling the LLM? My lean:
   yes — the comparator's choice should reflect what the LLM would
   have done in the same routing context. Confirm.

4. **Corpus retention soft trigger.** Layer 1 records may include
   verbatim prompts. Retention policy: **revisit at ~1000 records
   OR 30 days of probe-on use, whichever first.** Until then, no
   automatic deletion; operator may prune manually. The trigger is
   a soft prompt to re-evaluate, not an automatic-delete rule.
   Confirm threshold values.

5. **Layer 2 regeneration semantics on model change.** When the
   comparator default model changes, do we regenerate all of Layer 2,
   or do we keep both old and new Layer 2 records side-by-side?
   My lean: keep both — the side-by-side comparison is itself
   evidence about the system's arbitration sensitivity to model
   choice. Confirm.

6. **Where does the comparator live?** Top-level `tools/` or under
   `forge_bridge/`? My lean: `forge_bridge/corpus/comparator.py` with
   a console-script entry, so the comparator is part of the package
   and benefits from the package's dep tree. Confirm. Note: even if
   the comparator code lives in the package, **invocation runs in a
   separate process** per I-3; package-location is independent of
   process-boundary.

These are bounded questions — five-minute decisions, not deep design.
The contract structure above does not change based on their answers.
