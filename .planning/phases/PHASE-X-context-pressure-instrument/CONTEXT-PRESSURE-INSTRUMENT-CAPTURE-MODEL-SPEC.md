...
- **Two implications, neither changing the measure-first discipline or the blind Q3 threshold:**
  ...
  2. The §2.3 op→(context, required-selection-type) map **wants to be pipeline-declared capability metadata** (extending `CapabilityDeclaration`), consumed by bridge — NOT reverse-engineered from hook source. Reading `~/GitHub/forge-pipeline/flame_hooks/forge_tools/` to derive §2.3 (this spec's provenance) is an interim that violates the ADR-000 no-sibling-internals boundary. **Room sharpening (2026-06-05): NOT a blanket.** Per §2.4, only the **nominal `isinstance` guards** (the dominant PySequence-rename class) declare faithfully as a type name; **duck-typed (`hasattr`) and cardinality (`len==1`) guards flatten lossily** if forced into a type-name declaration — and a resolver consuming a flattened guard would disagree with the live hook about the same selection (a deictic trust fracture). Land it as: **nominal declared now; heterogeneous guards stay in the hook until the contract can express predicates, not just types.** What pipeline declares is a *predicate descriptor*, not merely a type — and where the predicate can't yet be declared, the resolver emits `unresolved` + `reason_code="unreconstructable_guard"` rather than relocate or flatten-guess. The output contract is done; this declarable-predicate vocabulary is the missing input half.
