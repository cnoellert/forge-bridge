...
| `timeline` | segment-walk on `seg.selected` (`_selection()`); `clip.selected_segments` | **✅ VERIFIED PULL CLEAN (live, 2026-06-05, portofino, Flame 2026.2.2)** — `clip.selected_segments.get_value()` → `returns_list:true`, count **7**, every item `isinstance PySegment` + `has_record_in` (both guard styles reconstruct). Decisive contrast on the SAME live state: the old `bool(seg.selected)` walk (`_focus.py` pre-`3c8fc61`) returned **31** (ALL segments, incl. empty-name `''` — the corpus-poison source); `get_value()` returns exactly the **7** selected. Confirms the segment-walk→`get_value()` fix empirically (7 real vs 31 spurious). The §4.1 ⚠/SUSPECT is RETIRED — all three contexts now zero-asterisk pull-clean via `get_value()`. |
...
- **Two implications, neither changing the measure-first discipline or the blind Q3 threshold:**
  ...
  > **INVARIANT (bridge-resolver, conditional on BUILD)** — *promoted from contract "should" to bridge MUST; Orch-ratified 2026-06-05, DT grounding folded in.* If Q3 clears BUILD, the bridge resolver MUST author a **grounded** human-facing `message` for every unresolved/ambiguous result surfaced to a person — composed from the real resolution facts (the `candidates`, the unresolved referent), **NOT** model-invented prose. A `reason_code` with `message=None` is the v1.9 *"dispatch envelope, not an answer"* failure ([[project_chat_is_a_console_not_chat]]) recurring one architectural layer down — **structurally prohibited, not a quality nicety.** Because the message is *grounded* (an explanation of state that exists, inside the v1.9 cut-line), this invariant **reinforces** the no-synthesis law ([[feedback_orchestrator_control_flow_not_meaning]]) rather than colliding with it. Scope = bridge's own resolver (within boundary; bridge governs its own component). The **federation-wide** version — binding every peer's resolver — is a **deferred contracts move**: it would be an ADR-000 constraint on the `message` field, contracts' to mint via the same round-trip that produced `44f8488`. Named, gated, not acted.
