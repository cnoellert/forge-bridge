# AGENTS.md — forge-bridge Context Recovery

This file exists to get an AI assistant (Codex or otherwise) back up to speed on this project quickly when context is lost between sessions. Read this first, then read the docs in order listed below.
...
- **Phase 20** (current) — Reality Audit + Canonical Install: ships `docs/INSTALL.md`, refreshes README install section + this `AGENTS.md` ground-truth, pins `install-flame-hook.sh` default to `v1.4.1`. Forcing function: a non-author UAT must pass on a fresh conda env.
...
- **Phase 22** — Daily Workflow Recipes: 6 step-by-step recipes (first-time setup, Codex Desktop wiring, tool synthesis, Flame chat automation, staged-ops approval, manifest inspection).
